...
class SoundDeviceMicrophone(sr.AudioSource):
    ...
    def __enter__(self):
        if self.stream is not None:
            raise ValueError("이 오디오 소스는 이미 context manager 내부에서 사용 중입니다.")
        
        # RawInputStream으로 PCM 형태의 바이너리 데이터를 직접 받는다.
    ...
    def read(self, size):
        ...
        RawInputStream으로부터 size만큼 데이터를 읽어온다.
        """
        data, overflowed = self.stream.read(size)
        return data
...
def process_voice_input():
    global voice_recorded_text, latest_camera_frame, latest_analysis_result, chunk_index
    if voice_recorded_text.strip():
        ...
        # 가장 최근의 음성 chunk 파일을 사용
        latest_chunk_file = f"chunk_{chunk_index-1}.wav"
        answer = answer_wav(voice_recorded_text.strip(), latest_chunk_file)
